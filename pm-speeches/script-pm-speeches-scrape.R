...
abv_vol1 <- pdftools::pdf_text(pdf = "pm-speeches/primeministerata01unse.pdf")
...
tibble(
    content = abv_vol1[5:13] |>
                read_lines() |>
                str_squish()
) |>
  separate(col = content,
    sep = "(?<=[[:upper:]]) (?=[[:digit:]]{1,3}$)",
    into = c("title","pg")) |> 
  filter(title != "") |> 
  fill(pg,.direction =  "down") |> 
  ...
  filter(!is.na(last_pg))|> 
  filter(title != "INDEX") -> pmabv_speeches_vol1
...
pmap(
    ...
    get_all_speechs,data = abv_vol1
) |>
  list_rbind() -> abv_speech_text_df_3



write.csv(abv_speech_text_df_3,"pm-speeches/AtalBihari-Vajpayee-speeches.csv")
